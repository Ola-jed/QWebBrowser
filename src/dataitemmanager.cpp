...
DataItemManager::DataItemManager()
{
    ...
    if(myDb.open())
    {
        ...
        queryCreation.exec("CREATE TABLE IF NOT EXISTS History ( Id INTEGER PRIMARY KEY AUTOINCREMENT,Date VARCHAR(30),Title VARCHAR(100),Url TEXT);");
        queryCreation.exec("CREATE TABLE IF NOT EXISTS Favorite ( Id INTEGER PRIMARY KEY AUTOINCREMENT,Date VARCHAR(30),Title VARCHAR(100),Url TEXT);");
    }
}
...
QList<DataItem> DataItemManager::selectAllFromHistory()
{
    ...
    while(queryAll.next())
    {
        histLoaded.push_back(DataItem{queryAll.value(0).toString(),queryAll.value(1).toString(),queryAll.value(2).toString()});
    }
    return histLoaded;
}
